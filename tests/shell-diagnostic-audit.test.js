...
async function syntaxError(command, commands, prior = []) {
...
async function feature(command, detail, commands, prior = []) {
  const result = await terminal(commands).run(command)
...
describe('compound assignment diagnostics use the command operand position', () => {
  for (const command of [
    ...
    it(command, async () => { await feature(command, 'array assignment') })
  }
  ...
  for (const command of [
    ...
    it(`ordinary syntax: ${command}`, async () => { await syntaxError(command) })
  }

  it('lets custom declaration commands receive quoted arguments normally', async () => {
    const result = await terminal({ declare: ({ args }) => args.join('|') }).run(`declare 'a=(one two)'`)
...
describe('alias diagnostic candidates follow input boundaries and scopes', () => {
  for (const [command, prior] of [
    ...
    it(`ordinary syntax: ${command}`, async () => { await syntaxError(command, undefined, prior) })
  }
  ...
  for (const command of [
    ...
    it(`alias gap: ${command}`, async () => {
      const unaliasRuns = /\n(?:unalias -x|\(unalias|\{ unalias)/u.test(command)
      await feature(command, 'alias expansion', undefined, unaliasRuns ? ['alias', 'unalias'] : ['alias'])
    })
  }

  it('retains alias evidence across boundaries that its replacement can change', async () => {
  ...
  it('does not assign builtin behavior to a custom alias command', async () => {
    await syntaxError("{ alias LEFT='('; }\nLEFT echo one )", { alias: () => '' })
  })

  it('does not remove builtin alias candidates through custom unalias commands', async () => {
    await feature("alias LEFT='('\nunalias LEFT\nLEFT echo one )", 'alias expansion', { unalias: () => '' }, ['alias'])
  })

  it('keeps possible definitions when a custom command status is unknown while parsing', async () => {
    await feature("probe && alias LEFT='('\nLEFT echo one )", 'alias expansion', { probe: () => '' }, ['alias'])
  })

  it('does not mistake a dynamic alias name for a literal definition', async () => {
    await syntaxError('alias "$name=("\nname echo one )', undefined, ['$name', 'alias'])
  })

  it('retains non-ASCII alias names that Bash allows without quoting', async () => {
...
describe('deferred command substitutions preserve diagnostic and quoting boundaries', () => {
  for (const inner of ['echo @(a|b)', 'alias LEFT=(one two)']) {
    it(`validates skipped substitution syntax: ${inner}`, async () => {
      const result = await terminal().run(`true || printf '%s' "$(${inner})"`)
  ...
  for (const pattern of ['@', '!', '?', '+', '*']) {
    it(`quotes and substitutions do not create ${pattern}( syntax`, async () => {
      const result = await terminal().run(`value='${pattern}(a|b)'; printf '%s' "$(printf '%s' "$value")"`)
  ...
  it('reports inner extglob syntax before applying enclosing redirects', async () => {
    const result = await terminal().run(`{ printf '%s' "$(echo @(a|b) 2>/dev/null)"; } 2>/dev/null | cat`)
    assert.equal(result.stdout, '')
    assert.notEqual(result.stderr, '')
