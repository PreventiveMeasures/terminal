...
describe('failed argument expansion never applies command redirects', () => {
  for (const expansion of ['$((1/0))', '${missing:?required}']) {
    for (const command of [
      ...
      it(command, async () => {
        const t = terminal()
        await t.run('echo keep >/tmp/target')
        const result = await t.run(command + '; echo lost')
        assert.equal(result.stdout, '')
        assert.notEqual(result.exitCode, 0)
        assert.notEqual(result.stderr, '')
        assert.deepEqual(await t.run('cat /tmp/target'), success('keep\n'))
      })
    }

    it(`does not evaluate redirect operands after ${expansion} fails`, async () => {
      const t = terminal()
      const result = await t.run(`echo "${expansion}" >"$((counter=1))" >"${'${target:=/tmp/other}'}"`)
      assert.notEqual(result.exitCode, 0)
      assert.deepEqual(await t.run('argv "${counter:-unset}" "${target:-unset}"'), success('["unset","unset"]'))
    })

    it(`only enclosing redirects can hide the error from ${expansion}`, async () => {
      const direct = await terminal().run(`echo "${expansion}" 2>/dev/null`)
      assert.notEqual(direct.stderr, '')
      const enclosed = await terminal().run(`{ echo "${expansion}"; } 2>/dev/null`)
...
describe('pathname expansion follows all argument substitutions', () => {
  ...
  it('an earlier glob observes a later removal', async () => {
    const t = terminal()
    await t.run('printf hi >/tmp/a.txt')
    assert.deepEqual(await t.run('argv /tmp/*.txt "$(rm /tmp/a.txt)"'), success('["/tmp/*.txt",""]', ['glob: no paths matched "/tmp/*.txt"; the pattern was left literal.']))
  })
})
...
describe('temporary assignments preserve other expansion side effects', () => {
  ...
  it('keeps earlier RHS side effects when a later prefix expansion fails', async () => {
    const t = terminal()
    const result = await t.run('n=0; X=$((n++)) Y=${missing:?required} true; echo lost')
    assert.equal(result.stdout, '')
    assert.notEqual(result.exitCode, 0)
    assert.deepEqual(result.unsupported, [])
    assert.deepEqual(await t.run('argv "$n" "${X-unset}" "${Y-unset}"'), success('["1","unset","unset"]'))
  })
  ...
  for (const command of [
    ...
    it(`diagnoses mutations of temporary assignment targets: ${command}`, async () => {
      for (const wrap of [(source) => source, (source) => `{ ${source}; } 2>/dev/null`]) {
        const result = await terminal().run(wrap(command))
        assert.notEqual(result.exitCode, 0)
...
describe('continued process substitutions cannot become literal fallback text', () => {
  for (const direction of ['<', '>']) {
    ...
    it(`diagnoses ${JSON.stringify(operand)} when selected`, async () => {
      const result = await terminal().run('argv ${missing:-' + operand + '}')
      assert.equal(result.stdout, '')
      assert.notEqual(result.exitCode, 0)
      assert.ok(result.unsupported.some((entry) => entry.detail === direction + '('))
    })
    it('leaves quoted operands literal and skips unselected ones', async () => {
      assert.deepEqual(await terminal().run('argv "${missing:-' + operand + '}"'), success(JSON.stringify([direction + '(printf lost)'])))
      assert.deepEqual(await terminal().run('value=kept; argv ${value:-' + operand + '}'), success('["kept"]'))
    })
  }
})

describe('IFS presence reflects its default and explicit bindings', () => {
  it('default IFS is set, but explicit unset remains absent', async () => {
    const t = terminal()
    assert.deepEqual(await t.run('[[ -v IFS ]] && argv "${IFS+set}" "${IFS:-fallback}"'), success(JSON.stringify(['set', ' \t\n'])))
    assert.deepEqual(await t.run('unset IFS; [[ -v IFS ]] || argv "${IFS+set}" "${IFS-fallback}"'), success('["","fallback"]'))
    assert.deepEqual(await t.run('IFS=; [[ -v IFS ]] && argv "${IFS+set}" "${IFS:-fallback}"'), success('["set","fallback"]'))
  })
})
