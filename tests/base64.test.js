...
describe('base64 standard text and byte encoding', () => {
  for (const [text, encoded] of vectors) {
    it(JSON.stringify(text), async () => {
      ...
      assert.deepEqual(await t.run('base64 input'), expected(encoded ? encoded + '\n' : ''))
      assert.deepEqual(await t.run('base64 -w0 input'), expected(encoded))
      assert.deepEqual(await t.run('base64 -d encoded'), expected(text))
    })
  }
  it('accepts stdin, explicit dash, redirections, and the long decode alias', async () => {
    const t = createTerminal({ input: 'foo', encoded: 'Zm9v' })
    for (const command of ['cat input | base64', 'cat input | base64 -', 'base64 <input']) {
      assert.deepEqual(await t.run(command), expected('Zm9v\n'))
    }
    assert.deepEqual(await t.run('base64 --decode encoded'), expected('foo'))
    assert.deepEqual(await t.run('base64 input | base64 --decode'), expected('foo'))
  })
  it('processes long inputs without depending on a function argument limit', async () => {
    const text = 'foo'.repeat(100_000)
    const t = createTerminal({ input: text })
    assert.deepEqual(await t.run('base64 -w0 input'), expected('Zm9v'.repeat(100_000)))
    assert.deepEqual(await t.run('base64 input | base64 -d'), expected(text))
  })
  it('stores encoded and decoded files entirely in the virtual filesystem', async () => {
    const t = createTerminal({ input: '{"ok":true}\n' }, { mount: '/src/', writable: '/tmp/' })
    assert.deepEqual(await t.run('base64 /src/input >/tmp/data; base64 -d /tmp/data >/tmp/plain; cat /tmp/plain'), mounted('{"ok":true}\n'))
  })
})

describe('base64 wrapping options', () => {
  const t = () => createTerminal({ input: 'a'.repeat(58), short: 'foo' })
  it('wraps after 76 columns by default', async () => {
    assert.deepEqual(await t().run('base64 input'), expected('YWFh'.repeat(19) + '\nYQ==\n'))
  })
  it('does not insert an extra blank line at an exact wrap boundary', async () => {
    assert.deepEqual(await createTerminal({ input: 'a'.repeat(57) }).run('base64 input'), expected('YWFh'.repeat(19) + '\n'))
  })
  for (const option of ['-w2', '-w 2', '--wrap=2', '--wrap 2', "-w ' +002'"]) {
    it(option, async () => assert.deepEqual(await t().run(`base64 ${option} short`), expected('Zm\n9v\n')))
  }
  for (const option of ['-w0', '--wrap=0', '-w -0', '-w 18446744073709551616']) {
    it(option, async () => assert.deepEqual(await t().run(`base64 ${option} short`), expected('Zm9v')))
  }
  it('a representable huge width still terminates its final line', async () => {
    assert.deepEqual(await t().run('base64 -w9223372036854775807 short'), expected('Zm9v\n'))
  })
  it('applies the last wrap alias regardless of spelling', async () => {
    assert.deepEqual(await t().run('base64 -w1 --wrap=0 short'), expected('Zm9v'))
    assert.deepEqual(await t().run('base64 --wrap=0 -w1 short'), expected('Z\nm\n9\nv\n'))
  })
  it('validates but otherwise ignores wrap during decoding', async () => {
    assert.deepEqual(await createTerminal({ input: 'Zm9v' }).run('base64 -dw1 input'), expected('foo'))
  })
...
describe('base64 decoder validates padding and preserves decoded prefixes', () => {
  ...
  for (const [input, stdout] of invalid) {
    it(`invalid ${JSON.stringify(input)}`, async () => {
      assert.deepEqual(await createTerminal({ input }).run('base64 -d input'), expected(stdout, 1, 'base64: invalid input\n'))
    })
  }
  it('retains stdout before stderr when their descriptors are combined', async () => {
    assert.deepEqual(await createTerminal({ input: 'Zm9v!' }).run('base64 -d input 2>&1'), expected('foobase64: invalid input\n', 1))
  })
  for (const option of ['-di', '-d -i', '--decode --ignore-garbage']) {
    it(option, async () => {
      assert.deepEqual(await createTerminal({ input: 'Y\t Q!\0=é=\r\n' }).run(`base64 ${option} input`), expected('a'))
    })
  }
  it('ignore-garbage does not discard padding or fix partial padding', async () => {
    assert.deepEqual(await createTerminal({ input: 'YQ=!\n' }).run('base64 -di input'), expected('a', 1, 'base64: invalid input\n'))
  })
  it('ignore-garbage has no effect while encoding', async () => {
    assert.deepEqual(await createTerminal({ input: 'foo' }).run('base64 -i input'), expected('Zm9v\n'))
  })
})
...
describe('base64 input and diagnostic errors', () => {
  for (const command of ['base64 missing', 'base64 -d missing', 'base64 dir']) {
    it(command, async () => {
      const result = await createTerminal({ 'dir/file': 'foo' }).run(command)
  ...
  for (const option of ['--unknown', '--base64url', '-u']) {
    it(option, async () => {
      ...
      assert.deepEqual(await t.run(`base64 ${option} input`), expected('', 1, message + '\n', unsupported))
  ...
  it('completes a UTF-8 character across separately padded blocks', async () => {
    assert.deepEqual(await createTerminal({ encoded: 'ww==qQ==' }).run('base64 -d encoded'), expected('é'))
  })
})

describe('base64 repeated decoding keeps each result independent', () => {
  it('does not retain bytes or errors from a previous longer decode', async () => {
    const t = createTerminal({ long: 'AAEC'.repeat(4096), partial: 'YQ==!', short: 'AQ==', empty: '' })
    assert.deepEqual(await t.run('base64 -d long'), expected('\0\u0001\u0002'.repeat(4096)))
    assert.deepEqual(await t.run('base64 -d partial'), expected('a', 1, 'base64: invalid input\n'))
    assert.deepEqual(await t.run('base64 -d short'), expected('\u0001'))
    assert.deepEqual(await t.run('base64 -d empty'), expected(''))
    assert.deepEqual(await t.run('base64 -d long'), expected('\0\u0001\u0002'.repeat(4096)))
  })
})
