...
describe('complete shell input units run before later parse failures', () => {
  for (const failure of ['echo )', 'echo "', 'echo `unterminated', 'echo @(bad)', 'echo ${!value}']) {
    it(`retains prior unsupported diagnostics before ${failure}`, async () => {
      const result = await terminal().run(`set -o pipefail\necho before\n${failure}`)
  ...
  it('preserves completed variable, directory and file changes', async () => {
    const t = terminal()
    const result = await t.run('value=kept\ncd /src/dir\nprintf written >/tmp/file\necho )')
    assert.equal(result.exitCode, 2)
    assert.equal(result.cwd, '/src/dir')
    assert.equal((await t.run('printf "%s:" "$value"; cat /tmp/file')).stdout, 'kept:written')
  })
  ...
  for (const command of [
    ...
    it(`does not execute an incomplete or invalid unit: ${command}`, async () => {
  ...
  it('does not diagnose runtime commands in skipped branches', async () => {
    const result = await terminal().run('false && set -o errexit\nif false; then set -u; fi\necho )')
    assert.equal(result.exitCode, 2)
    assert.deepEqual(result.unsupported, [])
  })

  for (const suffix of ['echo "', 'echo )', 'echo `unterminated', 'echo ${value:1}', 'echo $(echo ))']) {
    it(`does not read after exit: ${suffix}`, async () => {
      const result = await terminal().run('echo before\nexit 7\n' + suffix)
      assert.equal(result.stdout, 'before\n')
  ...
  it('still validates the complete semicolon list before exit', async () => {
    const result = await terminal().run('exit 7; echo )')
    assert.equal(result.exitCode, 2)
  })
})
...
describe('input boundaries follow grammar and lexical state', () => {
  ...
  it('blank and comment-only units preserve the prior status', async () => {
    assert.equal((await terminal().run('false\n\n# comment\n')).exitCode, 1)
  })

  it('checks active substitution syntax even in a skipped command', async () => {
    const result = await terminal().run('echo before\nfalse && echo $(echo ;)\necho after')
    assert.equal(result.stdout, 'before\nafter\n')
    assert.equal(result.stderr, '')
    assert.deepEqual(result.unsupported, [])
    const malformed = await terminal().run('echo before\nfalse && echo $(echo ;;)\necho after')
...
describe('malformed input boundaries never run a partial unit', () => {
  for (const command of [
    ...
    it(command, async () => {
      const t = terminal()
      await t.run('printf keep >/tmp/sentinel')
      const result = await t.run('echo before\n' + command)
      assert.equal(result.stdout, 'before\n')
      assert.equal(result.exitCode, 2)
      assert.deepEqual(result.unsupported, [])
      assert.equal((await t.run('cat /tmp/sentinel')).stdout, 'keep')
    })
  }
