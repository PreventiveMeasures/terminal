...
describe('cp copies regular files into the writable filesystem', () => {
  for (const [command, file] of [
    ...
    it(command, async () => {
      ...
      await check(t, `cat ${file}`, 'alpha\n')
      await check(t, 'cat a', 'alpha\n')
    })
  }

  it('copies multiple operands and preserves Unicode, NUL and empty content', async () => {
    const t = terminal()
    await check(t, 'cp a b empty /tmp')
    await check(t, 'cat /tmp/a /tmp/b /tmp/empty', 'alpha\nbeta\0😀')
    await check(t, 'ls /tmp', 'a\nb\nempty\n')
    assert.deepEqual(t.complete('cat /tmp/e'), ['cat /tmp/empty'])
  })

  it('copies globs and names beginning with an option prefix', async () => {
    const t = terminal()
    await check(t, 'cp -- -f /tmp/dash; cp one/* /tmp')
    await check(t, 'cat /tmp/dash /tmp/shared', 'literalfirst')
    assert.deepEqual(await t.run('cat a'), expected('alpha\n'))
  })

  it('overwrites rather than appending and leaves the source independent', async () => {
    const t = terminal()
    await check(t, 'printf longer-content >/tmp/out; cp a /tmp/out')
    await check(t, 'cat /tmp/out', 'alpha\n')
    await check(t, 'cp /tmp/out /tmp/second; printf changed >/tmp/out')
    await check(t, 'cat /tmp/second a', 'alpha\nalpha\n')
  })

  it('preserves existing destination inode identity for open descriptors', async () => {
    const t = terminal()
    await check(t, 'printf old >/tmp/out; { cp a /tmp/out; printf extra; } >>/tmp/out')
    await check(t, 'cat /tmp/out', 'alpha\nextra')
  })

  it('works through find and xargs dispatch without consuming their stdin again', async () => {
    const t = terminal()
    await check(t, "find dir -type f -exec cp {} /tmp ';'")
    await check(t, 'printf "a b" | xargs cp -t /tmp')
    await check(t, 'cat /tmp/leaf /tmp/a /tmp/b', 'leafalpha\nbeta\0😀')
    await check(t, 'printf retained | { cp a /tmp/copy; cat; }', 'retained')
  })
})
...
describe('cp no-clobber, verbosity, and multi-source conflicts', () => {
  ...
  it('prints verbose paths before an ordinary copy failure', async () => {
    await check(terminal(), 'cp --verbose a /repo/new', "'a' -> '/repo/new'\n", "cp: cannot create regular file '/repo/new': Read-only file system\n", 1)
  })

  it('does not overwrite a target produced by an earlier source with the same basename', async () => {
    const t = terminal()
    await check(t, 'cp -v one/shared two/shared /tmp', "'one/shared' -> '/tmp/shared'\n", "cp: will not overwrite just-created '/tmp/shared' with 'two/shared'\n", 1)
  ...
  it('warns about an identical repeated source without making success fail', async () => {
    const t = terminal()
    await check(t, 'cp -v a a /tmp', "'a' -> '/tmp/a'\n", "cp: warning: source file 'a' specified more than once\n")
    await check(t, 'cat /tmp/a', 'alpha\n')
  })

  it('recognizes different path spellings of the same repeated source', async () => {
    const t = terminal()
    await check(t, 'cp -v a ./a /repo//a /tmp', "'a' -> '/tmp/a'\n", "cp: warning: source file './a' specified more than once\ncp: warning: source file '/repo//a' specified more than once\n")
    await check(t, 'cat /tmp/a', 'alpha\n')
  })

  it('diagnoses buffered verbose output pointing at the source', async () => {
    const t = terminal()
    await check(t, 'printf old >/tmp/source')
    const result = await t.run('cp -v /tmp/source /tmp/dest >>/tmp/source')
    assert.equal(result.exitCode, 1)
    assert.deepEqual(result.unsupported.map(({ detail }) => detail), ['copy output buffering'])
    await check(t, 'cat /tmp/source', 'old')
    await check(t, 'test -e /tmp/dest', '', '', 1)
  })

  it('diagnoses buffered verbose output pointing at the destination', async () => {
    const t = terminal()
    const result = await t.run('cp -v a /tmp/out >/tmp/out')
    assert.equal(result.exitCode, 1)
    assert.deepEqual(result.unsupported.map(({ detail }) => detail), ['copy output buffering'])
    await check(t, 'cat /tmp/out', '')
  })

  it('leaves a copy that refuses before it writes to say so itself', async () => {
    const t = terminal()
    await check(t, 'printf old >/tmp/out')
  ...
  it('leaves an operand a just-created name refuses out of the overlap', async () => {
    const t = terminal()
    await check(t, 'mkdir -p /tmp/one /tmp/two /tmp/d; printf ONE >/tmp/one/x; printf TWO >/tmp/two/x')
    ...
    const refused = await t.run('cp -v /tmp/one/x /tmp/two/x /tmp/d >/tmp/one/x')
    assert.deepEqual(refused.unsupported.map(({ detail }) => detail), ['copy output buffering'])
  })

  it('leaves a destination it cannot write out of the overlap', async () => {
    const t = terminal()
  ...
  it('quotes unusual filenames consistently with other file commands', async () => {
    const name = 'a\nspace \'quote'
    const t = createTerminal({ [name]: 'content' }, { mount: '/repo', cwd: '/repo', writable: '/tmp/' })
    const r = await t.run(`cp -v ${quote(name)} /tmp/out`)
    assert.equal(r.exitCode, 0)
    assert.equal(r.stderr, '')
    assert.deepEqual(r.unsupported, [])
    assert.equal(r.stdout.split('\n').length, 2)
    assert.match(r.stdout, /\\n/u)
    await check(t, 'cat /tmp/out', 'content')
  })
})
...
describe('cp reports ordinary path, operand, and read-only failures', () => {
  ...
  it('reports self-copy without truncating even when forced', async () => {
    const t = terminal()
    await check(t, 'cp a /tmp/out')
    await check(t, 'cp -fv /tmp/out /tmp/./out', '', "cp: '/tmp/out' and '/tmp/./out' are the same file\n", 1)
    await check(t, 'cat /tmp/out', 'alpha\n')
  })

  it('continues copying later sources after a missing source or directory', async () => {
    const t = terminal()
    await check(t, 'cp -v missing a dir b /tmp', "'a' -> '/tmp/a'\n'b' -> '/tmp/b'\n", "cp: cannot stat 'missing': No such file or directory\ncp: -r not specified; omitting directory 'dir'\n", 1)
    await check(t, 'cat /tmp/a /tmp/b', 'alpha\nbeta\0😀')
  })
...
describe('cp unsupported features and write guards retain diagnostics', () => {
  ...
  it('cannot bypass a parent xargs input guard with a newly opened destination', async () => {
    const t = terminal()
    await check(t, 'printf "/tmp/args\\n" >/tmp/args')
    const r = await t.run('xargs -I{} cp a {} </tmp/args 2>/dev/null | cat')
    assert.equal(r.stdout, '')
    assert.equal(r.stderr, '')
    assert.equal(r.exitCode, 0)
