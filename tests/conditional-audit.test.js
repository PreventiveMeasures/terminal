...
describe('conditional quoting survives continuations and concatenated words', () => {
  for (const [source, status] of [
    ...
    it(source, async () => { assert.deepEqual(await terminal().run(source), expected(status)) })
  }

  it('consumes original source positions after removing local continuations', async () => {
    const source = '[[ $\\\n"one" == one ]]; printf before; [[ $\\\n"two" == two ]] && printf after'
    assert.deepEqual(await terminal().run(source), expected(0, 'beforeafter'))
  })

  it('keeps continuation recognition in a nested command substitution', async () => {
    assert.deepEqual(await terminal().run('printf "%s" "$(if [[ $\\\n"foo" == foo ]]; then printf yes; fi)"'), expected(0, 'yes'))
  })
})
...
describe('conditional predicates retain the Bash expression grammar', () => {
  for (const [expression, status] of [
    ...
    it(expression, async () => { assert.deepEqual(await terminal().run(`[[ ${expression} ]]`), expected(status)) })
  }
  ...
  for (const source of [
    ...
    it(`diagnoses ${source}`, async () => {
      const result = await terminal().run(source)
...
describe('conditional operands expand in order with lazy state and input consumption', () => {
  for (const [source, status, stdout, notes] of [
    ...
    it(source, async () => { assert.deepEqual(await terminal().run(source), expected(status, stdout, '', notes)) })
  }

  it('preserves ordinary inner command errors without inventing unsupported notes', async () => {
    const result = await terminal().run('[[ ! -n "$(cat absent)" ]]')
  ...
  it('applies conditional redirects before expanding command substitutions', async () => {
    assert.deepEqual(await terminal().run('[[ ! -n "$(cat absent)" ]] 2>/dev/null'),
      expected(0, '', '', ['cat: No such file or directory: "absent".']))
    assert.deepEqual(await terminal().run('[[ "$(cat)" == input ]] < data'), expected())
  })
})
