...
const check = async (t, command, stdout = '') => assert.deepEqual(await t.run(command), result(stdout), command)
...
describe('writable redirects are visible while enclosing commands execute', () => {
  ...
  for (const [command, content] of cases) {
    it(command, async () => {
      ...
      await check(t, 'cat /tmp/log', content)
    })
  }
  it('a command substitution reads diagnostics written by earlier inner commands', async () => {
    const t = terminal()
    await check(t, '{ value=$(cat missing; cat /tmp/errors); printf "%s" "$value"; } 2>/tmp/errors', ERROR.trimEnd())
    await check(t, 'cat /tmp/errors', ERROR)
  })
  it('a substitution can duplicate its captured stdout over inherited file stderr', async () => {
    const t = terminal()
  ...
  it('inner redirects override and then restore the enclosing file', async () => {
    const t = terminal()
    await check(t, '{ printf A; printf B >/tmp/other; printf C | cat >>/tmp/other; printf D; } >/tmp/log')
    await check(t, 'cat /tmp/log', 'AD')
    await check(t, 'cat /tmp/other', 'BC')
  })
  it('delegated find commands see writes from earlier child commands', async () => {
    const t = terminal()
    const command = 'find /repo/input -exec printf A \\; -exec grep -q A /tmp/log \\; -exec printf yes \\; >/tmp/log'
    await check(t, command)
    await check(t, 'cat /tmp/log', 'Ayes')
  })
  it('find prints precede child commands that inspect their output file', async () => {
    const t = terminal()
    await check(t, 'find /repo/input -print -exec grep -q input /tmp/log \\; -exec printf yes \\; >/tmp/log')
    await check(t, 'cat /tmp/log', '/repo/input\nyes')
  })
  it('find print0 and child output keep their action order', async () => {
    const t = terminal()
    await check(t, 'find /repo/input -print0 -exec printf middle \\; -print0 >/tmp/log')
    await check(t, 'cat /tmp/log', '/repo/input\0middle/repo/input\0')
  })
  it('find errors reach inherited stderr before later child commands read it', async () => {
    const t = terminal()
    const actual = await t.run('find missing /repo/input -exec grep -q missing /tmp/log \\; -exec printf yes \\; 2>/tmp/log')
    assert.deepEqual(actual, result('yes', 1))
    await check(t, 'cat /tmp/log', 'find: \'missing\': No such file or directory\n')
  })
  it('find known stdout and stderr events can share a file in action order', async () => {
    const t = terminal()
    const actual = await t.run('find missing /repo/input -print 2>&1 >/tmp/out')
    assert.deepEqual(actual, result('find: \'missing\': No such file or directory\n', 1))
    await check(t, 'cat /tmp/out', '/repo/input\n')
    assert.deepEqual(await t.run('find missing /repo/input -print >/tmp/log 2>&1'), result('', 1))
    await check(t, 'cat /tmp/log', 'find: \'missing\': No such file or directory\n/repo/input\n')
  })
  it('delegated xargs commands see writes from earlier child commands', async () => {
    const t = terminal({ commands: {
      snapshot: ({ args, fs }) => fs.readFile('/tmp/log') + args[0],
    } })
    await check(t, "printf 'a b' | xargs -n1 snapshot >/tmp/log")
    await check(t, 'cat /tmp/log', 'aab')
  })
})
...
describe('writable file descriptions retain independent offsets', () => {
  ...
  it('opens every output target even when a later redirect replaces it', async () => {
    const t = terminal()
    await check(t, 'printf old >/tmp/first; printf new >/tmp/first >/tmp/second')
    await check(t, 'cat /tmp/first')
    await check(t, 'cat /tmp/second', 'new')
  })
  it('retains earlier truncation when a later target fails to open', async () => {
    const t = terminal()
    await check(t, 'printf old >/tmp/log')
    const failed = await t.run('printf new >/tmp/log >/tmp/missing/file')
  ...
  it('does not open redirects for an unexecuted gated command', async () => {
    const t = terminal()
    await check(t, 'printf old >/tmp/log; false && printf new >/tmp/log; cat /tmp/log', 'old')
  })
  it('expands command arguments before truncating the output target', async () => {
    const t = terminal()
    await check(t, 'printf old >/tmp/log; printf "%s" "$(cat /tmp/log)" >/tmp/log')
    await check(t, 'cat /tmp/log', 'old')
  })
})
...
describe('fresh captures and reopened output do not inherit closed stdout', () => {
  ...
  it('still reports a direct write to an inherited closed stdout', async () => {
    const actual = await terminal().run('{ printf X; } >&-')
...
describe('ambiguous stream order cannot silently corrupt writable files', () => {
  ...
  for (const redirects of ['>/tmp/log 2>&1', '>/tmp/log 2>/tmp/log', '>>/tmp/log 2>>/tmp/log']) {
    it(`diagnoses unspecified handler ordering for ${redirects}`, async () => {
      const t = terminal({ commands })
      const actual = await t.run(`mixed ${redirects} | cat`)
      assert.equal(actual.stdout, '')
      assert.equal(actual.stderr, '')
      assert.equal(actual.exitCode, 0)
      assert.equal(actual.unsupported.length, 1)
      assert.equal(actual.unsupported[0].detail, 'combined output ordering')
      const stored = await t.run('cat /tmp/log')
      assert.equal(stored.exitCode, 0)
      assert.match(stored.stdout, /merging.*stdout and stderr/u)
    })
  }
  it('independent output files do not require stream ordering', async () => {
    const t = terminal({ commands })
    await check(t, 'mixed >/tmp/out 2>/tmp/err')
    await check(t, 'cat /tmp/out', 'stdout')
    await check(t, 'cat /tmp/err', 'stderr\n')
  })
})
...
describe('writable input redirections cannot retain pre-truncation data', () => {
  for (const redirects of ['</tmp/log >/tmp/log', '>/tmp/log </tmp/log']) {
    it(`self-truncating input: ${redirects}`, async () => {
      const t = terminal()
      await check(t, 'printf old >/tmp/log')
      await check(t, `cat ${redirects}`)
      await check(t, 'cat /tmp/log')
    })
  }
  it('reads named operands after output redirection truncates the file', async () => {
    const t = terminal()
    await check(t, 'printf old >/tmp/log; cat /tmp/log >/tmp/log')
    await check(t, 'cat /tmp/log')
  })
  for (const command of [
    ...
    it(`updated inherited input is read correctly or explicitly diagnosed: ${command}`, async () => {
      const t = terminal()
      await check(t, 'printf old >/tmp/log')
  ...
  it('does not silently copy an input file into its own append destination', async () => {
    // GNU cat refuses a regular input positioned before its output append EOF:
    // https://github.com/coreutils/coreutils/blob/master/src/cat.c
    const t = terminal()
    await check(t, 'printf old >/tmp/log')
    const actual = await t.run('cat /tmp/log >>/tmp/log')
    assert.notEqual(actual.exitCode, 0)
    assert.equal(actual.stdout, '')
    assert.notEqual(actual.stderr, '')
    await check(t, 'cat /tmp/log', 'old')
