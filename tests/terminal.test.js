...
describe('createTerminal — complete: corner cases', () => {
  ...
  it('empty completion lists ls first, then by auditor priority', () => {
    ...
    assert.deepEqual(all.slice(idx('which')), ['which', String.raw`\[`, 'base32', 'basename', 'brotli', 'cp', 'date', 'dirname', 'egrep', 'false', 'fgrep', 'gunzip', 'gzcat', 'gzip', 'ln', 'mkdir', 'od', 'patch', 'rm', 'sed', 'sha1sum', 'sha256sum', 'sha384sum', 'sha512sum', 'shasum', 'tar', 'tee', 'touch', 'true', 'unzip', 'whoami', 'xxd', 'zcat', 'zip'])
  })
  ...
  it('after `|`, completion only suggests commands that consume stdin', () => {
    ...
    for (const name of ['base32', 'brotli', 'egrep', 'fgrep', 'gunzip', 'gzcat', 'gzip', 'od', 'sed', 'sha1sum', 'sha256sum', 'sha384sum', 'sha512sum', 'shasum', 'tar', 'tee', 'xxd', 'zcat']) {
      assert.ok(c.includes('cat | ' + name), `${name} should be a pipe target`)
    }
    assert.equal(c.length, 35)
  })
  ...
  it('inserts a space after a bare `|` but preserves a typed target', () => {
    ...
    const targets = [
      ...
      'base32', 'brotli', 'egrep', 'fgrep', 'gunzip', 'gzcat', 'gzip', 'od', 'sed', 'sha1sum', 'sha256sum', 'sha384sum', 'sha512sum', 'shasum', 'tar', 'tee', 'xxd', 'zcat',
    ]
...
describe('createTerminal — awk', () => {
  // A small "audit" corpus: a whitespace table, a colon table, and a
  // two-file pair for the NR==FNR idiom.
  const SRC = {
    'people.txt': 'ann 25 la\nbob 30 ny\ncid 35 la\n',
    'passwd': 'root:x:0:0:root:/root:/bin/bash\nann:x:1000:1000::/home/ann:/bin/zsh\n',
    'a.txt': 'x\ny\n',
    'b.txt': 'y\nz\n',
    'nonl.txt': 'p q\nr s',
    'blank.txt': 'one\n\n  \nfour\n',
    'prog.awk': '# count words\n{ n += NF }\nEND { print n }\n',
    'src/dir.txt': 'hi\n',
  }
  ...
  it('rejects every form that would spawn a process — at parse time, even in a branch that never runs', async () => {
    const cases = [
      ...
      ["awk 'BEGIN { cmd = \"date\"; cmd | getline d }'", /command pipelines/u],
      ["awk '{ print $1 | \"sort\" }' a.txt", /output pipes \(`print \.\.\. \| "cmd"`\) are not supported: this terminal runs no processes/u],
