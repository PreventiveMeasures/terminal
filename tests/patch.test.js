...
function overlay(files = TREES.patchable) {
  const t = createTerminal(files, { mount: '/src', cwd: '/src', writable: '/tmp/' })
  for (const name of Object.keys(files)) assert.equal(t.run(`cp '${name}' '/tmp/${name}'`).exitCode, 0, name)
  t.run('cd /tmp')
  return t
}
...
describe('patch refuses on the feed what it cannot do', () => {
  it('writing anywhere but the overlay', () => {
    const t = createTerminal(TREES.patchable)
    const r = t.run('diff -u ten ten2 > /dev/null; diff -u ten ten2 | patch ten')
    assert.deepEqual(feed(r), [['feature', 'patch', 'read-only target']])
    assert.equal(r.stdout, '')
    assert.equal(r.stderr, 'patch: ten: file system is read-only\n')
    assert.equal(r.exitCode, 2)
    assert.equal(t.run('cat ten').stdout, TREES.patchable.ten, 'the file is untouched')
  })
  it('writing anywhere but the overlay, with an overlay elsewhere', () => {
    const t = createTerminal(TREES.patchable, { mount: '/src', cwd: '/src', writable: '/tmp/' })
    const r = t.run('diff -u ten ten2 | patch 2>/dev/null')
    assert.deepEqual(feed(r), [['feature', 'patch', 'read-only target']])
    assert.equal(t.run('diff -u ten ten2 | patch -o /tmp/out; cat /tmp/out').stdout, 'patching file /tmp/out (read from ten)\n' + TREES.patchable.ten2)
  })
  it('but checks and prints without writing anywhere', () => {
    const t = createTerminal(TREES.patchable)
    assert.deepEqual(t.run('diff -u ten ten2 | patch --dry-run').stdout, 'checking file ten\n')
    const out = t.run('diff -u ten ten2 | patch -o - ten')
  ...
  for (const [line, detail] of [
    ...
    it(line, () => {
      const r = createTerminal(TREES.patchable).run(line)
      ...
      assert.deepEqual(feed(createTerminal(TREES.patchable).run(line + ' 2>/dev/null | cat')), [['option', 'patch', detail]].map(([, c, d]) => [r.unsupported[0].kind, c, d]), line)
    })
  }
  it('keeps what it said before refusing', () => {
    const t = overlay()
    const r = t.run("printf -- '--- ten\\n+++ ten\\n@@ -1 +1 @@\\n-a\\n+A\\n--- x\\n+++ x\\n@@ -1 +1 @@\\r\\n-a\\r\\n+A\\r\\n' | patch")
    assert.equal(r.stdout, 'patching file ten\n')
    assert.deepEqual(feed(r), [['feature', 'patch', 'CRLF patch']])
    assert.equal(t.run('head -1 ten').stdout, 'A\n', 'the first file was written before the refusal')
  })
})
...
describe('what diff writes, patch applies, in every style', () => {
  ...
  for (const style of styles) {
    for (const [from, to] of pairs) {
      it(`diff ${style} ${from} ${to} | patch, and back with -R`, () => {
        const t = overlay()
        const forward = t.run(`diff ${style} ${from} ${to} > p; cp ${from} work; patch work < p`)
        assert.equal(forward.exitCode, 0, forward.stdout + forward.stderr)
        assert.equal(t.run('cat work').stdout, TREES.patchable[to])
        const back = t.run('patch -R work < p')
        assert.equal(back.exitCode, 0, back.stdout + back.stderr)
        assert.equal(t.run('cat work').stdout, TREES.patchable[from])
        assert.equal(t.run('test -e work.rej || test -e work.orig || echo neither').stdout, 'neither\n', 'a clean application makes no backup and no reject')
      })
    }
  }
  it('finds hunks that moved, and says so', () => {
    const t = overlay()
    const r = t.run("diff -u big big2 > p; printf 'top\\nnext\\n' > work; cat big >> work; patch work < p")
    assert.equal(r.stdout, 'patching file work\nHunk #1 succeeded at 4 (offset 2 lines).\nHunk #2 succeeded at 20 (offset 2 lines).\n')
    assert.equal(t.run('cat work').stdout, 'top\nnext\n' + TREES.patchable.big2)
    assert.equal(t.run('ls work.orig').stdout, 'work.orig\n', 'a mismatch keeps a backup')
  })
  it('applies a run of patches to one file in order', () => {
    const t = overlay()
    assert.equal(t.run("diff -u ten ten2 > p1; sed 's/^d$/D/' ten2 > ten3; diff -u -L ten -L ten ten2 ten3 > p2; cat p1 p2 | patch; cat ten").stdout, 'patching file ten\npatching file ten\na\nb\nX\nD\ne\nf\ng\nh\nY\nj\n')
  })
  it('appends rejects from a second patch to the same file', () => {
    const t = overlay()
    const r = t.run("printf -- '--- ten\\n+++ ten\\n@@ -1 +1 @@\\n-q\\n+Q\\n--- ten\\n+++ ten\\n@@ -2 +2 @@\\n-r\\n+R\\n' | patch; cat ten.rej")
    assert.equal(r.exitCode, 0)
    assert.equal(r.stdout, 'patching file ten\nHunk #1 FAILED at 1.\n1 out of 1 hunk FAILED -- saving rejects to file ten.rej\npatching file ten\nHunk #1 FAILED at 2.\n1 out of 1 hunk FAILED -- saving rejects to file ten.rej\n--- ten\n+++ ten\n@@ -1 +1 @@\n-q\n+Q\n--- ten\n+++ ten\n@@ -2 +2 @@\n-r\n+R\n')
  })
  it('-d applies inside another directory', () => {
    ...
    assert.equal(t.run('cp ten /tmp/ten; diff -u ten ten2 > /tmp/p; cd /; patch -d /tmp < /tmp/p; cat /tmp/ten').stdout, 'patching file ten\n' + TREES.patchable.ten2)
    assert.equal(t.run('patch -d /nowhere < /tmp/p').stderr, "patch: **** Can't change to directory /nowhere : No such file or directory\n")
  })
})
