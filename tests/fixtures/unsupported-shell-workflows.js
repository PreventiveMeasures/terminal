...
export const SHELL_WORKFLOWS = [
  ...
  {
    purpose: 'Use a shell conditional expression to find a package manifest',
    command: '[[ -f package.json ]] && cat package.json',
    expected: feature('[['),
    parseTime: true,
  },
  {
    purpose: 'Strip the directory from a source filename with parameter expansion',
    command: 'name=src/index.js; echo "${name##*/}"',
    expected: feature('${'),
    parseTime: true,
  },
  {
    purpose: 'Remove a JavaScript filename extension with parameter expansion',
    command: 'file=src/index.js; echo "${file%.js}"',
    expected: feature('${'),
    parseTime: true,
  },
  {
