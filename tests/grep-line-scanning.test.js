...
describe('grep line scanning', () => {
  it('counts and lists real empty lines without inventing one after a final newline', async () => {
    const operands = 'empty blank blanks terminated trailing unterminated leading'
    await check("grep -c '^$' " + operands, 'empty:0\nblank:1\nblanks:2\nterminated:0\ntrailing:1\nunterminated:0\nleading:1\n')
    await check("grep -l '^$' " + operands, 'blank\nblanks\ntrailing\nleading\n')
    await check("grep -L '^$' " + operands, 'empty\nterminated\nunterminated\n')
    await check("grep -vc '^$' blank terminated trailing unterminated leading", 'blank:0\nterminated:1\ntrailing:1\nunterminated:1\nleading:1\n')
  })

  it('honors selection caps and quiet status for empty and unterminated lines', async () => {
    await check("grep -cm1 '^$' blanks trailing", 'blanks:1\ntrailing:1\n')
    await check("grep -cm1 '^$' empty terminated unterminated", 'empty:0\nterminated:0\nunterminated:0\n', 1)
    await check('grep -cm1 hit terminated trailing unterminated leading', 'terminated:1\ntrailing:1\nunterminated:1\nleading:1\n')
    for (const name of ['blank', 'blanks', 'trailing', 'leading']) await check("grep -qm1 '^$' " + name, '')
  ...
  it('keeps binary and locale diagnostics after an early matching line', async () => {
    ...
    for (const mode of ['-q', '-l', '-L', '-cm1', '-m1']) {
      for (const [operands, detail, message] of cases) {
        ...
        assert.deepEqual(await createTerminal(inputs).run(command), {
