...
describe('the locale is C.UTF-8', () => {
  it('is what $LANG answers, in a terminal and in its forks', async () => {
    assert.deepEqual(await run('echo $LANG'), ok('C.UTF-8\n'))
    const t = createTerminal(FILES)
    assert.equal((await t.fork().run('echo $LANG')).stdout, 'C.UTF-8\n')
    assert.equal((await t.fork({ inherit: false, user: 'ada' }).run('echo $LANG')).stdout, 'C.UTF-8\n')
  })

  it('is the one value createTerminal takes, spelt as glibc spells it', async () => {
    for (const locale of ['C.UTF-8', 'C.utf8', 'c.UTF8']) assert.equal((await run('echo $LANG', { locale })).stdout, 'C.UTF-8\n', locale)
  ...
  it('takes an assignment that leaves the character set where it is', async () => {
    for (const line of [
    ...
    assert.equal((await run('LC_ALL=C.utf8; echo $LC_ALL $LANG')).stdout, 'C.utf8 C.UTF-8\n')
  })

  it('refuses an assignment that would move it, before the command runs', async () => {
  ...
  it('refuses unsetting LANG, which would hand the character set to C', async () => {
    const message = `unset: LANG: ${MESSAGE}`
    assert.deepEqual(await run('unset LANG'), ok('', { stderr: message + '\n', exitCode: 1, unsupported: [{ kind: 'feature', command: 'unset', detail: 'LANG', message }] }))
    assert.deepEqual(await run('unset LC_ALL; echo $LANG'), ok('C.UTF-8\n'))
  })

  it('runs what a refused assignment did not stop in C.UTF-8', async () => {
    const r = await run('LC_ALL=C; wc -m cafe')
