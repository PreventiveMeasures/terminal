...
async function gap(t, command, stderr) {
...
describe('touch makes the files it names', () => {
  it('creates an empty file', async () => {
    const t = terminal()
    await check(t, 'touch /tmp/new')
    await check(t, 'ls /tmp', 'new\n')
    await check(t, 'wc -c /tmp/new', '0 /tmp/new\n')
    await check(t, 'test -f /tmp/new && echo yes', 'yes\n')
  })

  it('creates every name it is given', async () => {
    const t = terminal()
    await check(t, 'touch /tmp/one /tmp/two /tmp/three')
    await check(t, 'ls /tmp', 'one\nthree\ntwo\n')
  })

  it('creates a file a later redirect can append to', async () => {
    const t = terminal()
    await check(t, 'touch /tmp/log; echo first >>/tmp/log; echo second >>/tmp/log')
    await check(t, 'cat /tmp/log', 'first\nsecond\n')
  })

  it('creates inside a directory cp -r made', async () => {
    const t = terminal()
    await check(t, 'cp -r dir /tmp/copy')
    await check(t, 'touch /tmp/copy/new')
    await check(t, 'ls /tmp/copy', 'leaf\nnew\n')
  })

  it('takes a name that looks like an option after --', async () => {
    const t = terminal()
    await check(t, 'touch -- /tmp/-c')
    await check(t, 'ls /tmp', '-c\n')
  })

  it('reads as a command, in completion and in which', async () => {
    const t = terminal()
    assert.deepEqual(t.complete('tou'), ['touch'])
    await check(t, 'which touch', '/usr/bin/touch\n')
  })
})

describe('touch reports the times it cannot set', () => {
  it('names a file that is already there', async () => {
    const t = terminal()
  ...
  it('names a directory that is already there', async () => {
    const t = terminal()
    await check(t, 'cp -r dir /tmp/copy')
    await gap(t, 'touch /tmp/copy', TIMES('/tmp/copy'))
    await gap(t, 'touch /tmp/copy/', TIMES('/tmp/copy/'))
    await gap(t, 'touch /tmp', TIMES('/tmp'))
  })

  it('reports it under -c as well, since -c still sets times', async () => {
    const t = terminal()
    await check(t, 'touch /tmp/new')
    await gap(t, 'touch -c /tmp/new', TIMES('/tmp/new'))
    await gap(t, 'touch --no-create /tmp/new', TIMES('/tmp/new'))
  })

  it('reports it once for a line that names the same gap twice', async () => {
    const t = terminal()
    await check(t, 'touch /tmp/one /tmp/two')
    await gap(t, 'touch /tmp/one /tmp/two', TIMES('/tmp/one') + TIMES('/tmp/two'))
  })

  it('creates what it can and reports what it cannot, in one line', async () => {
    const t = terminal()
    await check(t, 'touch /tmp/first')
    await gap(t, 'touch /tmp/first /tmp/second', TIMES('/tmp/first'))
    await check(t, 'ls /tmp', 'first\nsecond\n')
  })

  it('cannot be hidden by a redirect or a pipe', async () => {
    const t = terminal()
    await check(t, 'touch /tmp/new')
    const result = await t.run('touch /tmp/new 2>/dev/null | cat')
...
describe('touch reports a name it cannot reach the way GNU reports it', () => {
  for (const [command, stderr, exitCode] of [
    ...
    it(command, async () => {
      const t = terminal()
      await check(t, 'printf x >/tmp/file')
      await check(t, command, '', stderr, exitCode)
  ...
  it('points at a path that exists under another root', async () => {
    const t = terminal()
    const result = await t.run('touch dir/leaf/new')
    assert.equal(result.stderr, "touch: cannot touch 'dir/leaf/new': Not a directory\n")
    assert.equal(result.exitCode, 1)
    const noted = await terminal({ cwd: '/' }).run('touch dir/leaf')
  ...
  it('hints at nothing for a name -c passes over', async () => {
    const quiet = await terminal({ cwd: '/' }).run('touch -c dir/leaf')
    ...
    const reported = await terminal({ cwd: '/' }).run('touch dir/leaf')
...
describe('touch refuses the options it has no clock for', () => {
  for (const option of ['-a', '-m', '-d 2024-01-01', '-t 202401010000', '-r /repo/file', '--time=mtime', '-h']) {
    it(option, async () => {
      const t = terminal()
      const result = await t.run(`touch ${option} /tmp/new 2>/dev/null | cat`)
