...
describe('filesystem index and traversal', () => {
  it('normalizes source keys, and orders every kind of child by code point', () => {
    const fs = createFs({
      'z.txt': 'last',
      '\uE000': 'BMP',
      '😀/leaf': 'astral child',
      ...
      'a/./child': 'child',
      'root/../m.txt': 'middle',
      l: { type: 'symlink', target: 'a' },
    })
    // UTF-16 order would put both astral names before U+E000.
    assert.deepEqual(fs.listDir('/'), { dirs: ['a', '😀'], files: ['m.txt', 'z.txt', '\uE000', '😁'], links: ['l'] })
    assert.equal(fs.readFile('/a/child'), 'child')
    ...
    assert.deepEqual([...fs.walkFiles('/')], ['/a/child', '/m.txt', '/z.txt', '/\uE000', '/😀/leaf', '/😁'])
