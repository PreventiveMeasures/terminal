...
async function check(t, command, stdout = '', { stderr = '', exitCode = 0, notes = [], cwd = '/' } = {}) {
...
describe('a link is resolved the way the kernel resolves one', () => {
  it('reads the target from the directory the link itself is in', async () => {
    const t = terminal()
    await check(t, 'cat node_modules/.bin/cli', 'run\n')
    await check(t, 'cat pkg/index.js', 'module\n')
    await check(t, 'wc -c pkg/bin/cli.js node_modules/.bin/cli', '4 pkg/bin/cli.js\n4 node_modules/.bin/cli\n8 total\n')
  })
  ...
  it('reports a link that leads nowhere as the missing path it is', async () => {
    const t = terminal()
    await check(t, 'cat node_modules/.bin/stale', '', { stderr: 'cat: node_modules/.bin/stale: No such file or directory\n', exitCode: 1 })
    await check(t, 'cat node_modules/.bin/cli/deeper', '', { stderr: 'cat: node_modules/.bin/cli/deeper: Not a directory\n', exitCode: 1 })
  })
...
describe('find names a link, and passes over what it points at', () => {
  it('answers -type l beside -type f, which is the pairing a walk of node_modules needs', async () => {
    const t = terminal()
    await check(t, 'find node_modules -type f -o -type l',
      'node_modules/.bin/cli\nnode_modules/.bin/stale\nnode_modules/pkg/bin/cli.js\nnode_modules/pkg/index.js\n')
    await check(t, 'find . -type l', './node_modules/.bin/cli\n./node_modules/.bin/stale\n./pkg\n')
    await check(t, 'find . -type f -name "*.js"', './node_modules/pkg/bin/cli.js\n./node_modules/pkg/index.js\n./src/app.js\n')
    await check(t, 'find node_modules/.bin -mindepth 1 -type d')
  })

  it('answers over a tree that declares no link at all', async () => {
    const t = terminal({ 'a.txt': 'x\n' })
    await check(t, 'find . -type l')
    await check(t, 'find . -type f -o -type l', './a.txt\n')
    await check(t, 'find . -type l -o -type f', './a.txt\n')
  })

  it('reports the link it is given rather than the tree it names', async () => {
    const t = terminal()
...
describe('ls shows a link as the entry it is', () => {
  ...
  const dated = (sources) => at(async () => {
    const t = createTerminal(sources)
    await t.run('TZ=UTC')
    return t
  })

  it('lists the name, and marks it with `@` under -F', async () => {
    const t = terminal()
    await check(t, 'ls node_modules/.bin', 'cli\nstale\n')
    await check(t, 'ls -F node_modules/.bin', 'cli@\nstale@\n')
    await check(t, 'ls -F', 'node_modules/\npkg@\nsrc/\n')
  })
  ...
  it('names what a link points at in a long listing, with the length of that path as its size', async () => {
    ...
    assert.deepEqual((await at(() => t.run('ls -l far'))).stdout, `lrwxrwxrwx 1 user user 60 Sep 18 05:52 far -> ${'a'.repeat(60)}\n`)
    assert.deepEqual((await at(() => t.run('ls -l'))).stdout.split('\n')[0], 'total 8')
  })
  ...
  it('marks what a link points at in a long listing, which is where GNU puts the indicator', async () => {
    ...
    assert.deepEqual((await at(() => t.run('ls -F dir file'))).stdout, 'dir@\nfile@\n')
  })

  it('lists what a link leads to when it is named alone, and the link itself under -l, -F or -d', async () => {
    const t = terminal()
    await check(t, 'ls pkg', 'bin\nindex.js\n')
    await check(t, 'ls -d pkg', 'pkg\n')
    await check(t, 'ls -F pkg', 'pkg@\n')
    await check(t, 'ls pkg/', 'bin\nindex.js\n')
  })

  it('lists a link that leads nowhere, which is there however little it names', async () => {
    const t = terminal()
    await check(t, 'ls node_modules/.bin/stale', 'node_modules/.bin/stale\n')
    await check(t, 'ls node_modules/.bin/gone', '', { stderr: "ls: cannot access 'node_modules/.bin/gone': No such file or directory\n", exitCode: 2 })
  })
...
describe('the rest of the tree tools answer for a link without crossing it', () => {
  ...
  it('du measures the link rather than what it points at', async () => {
    const t = terminal()
    await check(t, 'du -b node_modules/.bin', '31\tnode_modules/.bin\n')
    await check(t, 'du -ab node_modules/.bin', '17\tnode_modules/.bin/cli\n14\tnode_modules/.bin/stale\n31\tnode_modules/.bin\n')
    await check(t, 'du -b node_modules/.bin/cli', '17\tnode_modules/.bin/cli\n')
    await check(t, 'du --inodes -s node_modules/.bin', '3\tnode_modules/.bin\n')
  ...
  it('stat describes the name it is given, and -L what that name leads to', async () => {
    const t = terminal()
    await check(t, 'stat -c "%F %s" node_modules/.bin/cli', 'symbolic link 17\n')
    await check(t, 'stat -c "%F" node_modules/.bin/stale', 'symbolic link\n')
    await check(t, 'stat -L -c "%F %s" node_modules/.bin/cli', 'regular file 4\n')
    await check(t, 'stat -c "%F" pkg', 'symbolic link\n')
    await check(t, 'stat -L -c "%F" pkg', 'directory\n')
  })

  it('test asks about the link with -L and -h, and about the target otherwise', async () => {
    const t = terminal()
    await check(t, 'test -L node_modules/.bin/cli && echo link', 'link\n')
    await check(t, 'test -h node_modules/.bin/stale && echo link', 'link\n')
    await check(t, 'test -f node_modules/.bin/cli && echo file', 'file\n')
...
describe('the shell expands and completes a link by its own name', () => {
  it('matches a link with a pattern, including one that leads nowhere', async () => {
    const t = terminal()
    await check(t, 'echo node_modules/.bin/*', 'node_modules/.bin/cli node_modules/.bin/stale\n')
  ...
  it('refuses a working directory reached through a link rather than answering as `cd -P`', async () => {
    const t = terminal()
    await gap(t, 'cd pkg', 'symbolic link cwd', 'cd: pkg: a working directory reached through a symbolic link is not supported (it leads to /node_modules/pkg)\n')
    await check(t, 'cd node_modules/pkg; pwd', '/node_modules/pkg\n', { cwd: '/node_modules/pkg' })
  })
})

describe('a search, a copy and a comparison each meet a link on their own terms', () => {
  it('grep -r passes over a link a walk reaches, where -R searches what it names', async () => {
    const t = terminal()
    await check(t, 'grep -rn run node_modules', 'node_modules/pkg/bin/cli.js:1:run\n')
    await check(t, 'grep -n run node_modules/.bin/cli', '1:run\n')
  ...
  it('grep -R refuses the one link it would have to walk into, unless a rule keeps it out', async () => {
    const t = terminal()
  ...
  it('rg leaves the links out of its walk, as ripgrep does without -L', async () => {
    const t = terminal()
    await check(t, 'rg -n run node_modules', 'node_modules/pkg/bin/cli.js:1:run\n', {
      notes: ['rg: skipped 1 hidden entry: "/node_modules/.bin". Hidden entries are searched with --hidden.'],
    })
    await check(t, 'rg -n --hidden run node_modules', 'node_modules/pkg/bin/cli.js:1:run\n')
  })

  it('cp reads through a link it is handed, which is what GNU does without -r', async () => {
  ...
  it('cp refuses a link a recursive copy meets, before that copy writes anything', async () => {
    ...
    const refused = await gap(t, 'cp -r a /tmp/copy', 'symbolic link', 'cp: copying a symbolic link is not supported: a/link (a recursive copy keeps the link, which cp does not make here)\n')
  ...
  it('diff names a link that is only on one side, and refuses only a walk that would cross one', async () => {
    ...
    await gap(t, 'diff -r a b', 'symbolic link to a directory', 'diff: comparing what a symbolic link to a directory holds is not supported: a/over\n')
  })
  ...
  it('diff reads a name a directory cannot answer for rather than typing it', async () => {
    ...
    await check(t, 'diff -r a b', '', { stderr: unreadable, exitCode: 2 })
...
describe('what a link cannot change', () => {
  ...
  it('refuses a write to a link naming a file under a directory that is not there', async () => {
    ...
    await check(after, 'find /tmp', '/tmp\n', { cwd: '/repo' })
  })
  ...
  it('writes through no link that leads nowhere, where GNU writes through neither half', async () => {
    ...
    await refused.run('cp file fresh')
  ...
  it('makes no directory over a name a link holds, however far the link leads', async () => {
    ...
    await check(await made(), 'mkdir /tmp/d; mkdir -p dirlink', '', at)
    await check(await made(), 'mkdir /tmp/d; mkdir -p dirlink/sub; find /tmp -type d', '/tmp\n/tmp/d\n/tmp/d/sub\n', at)
  ...
  it('answers for the parent of the name a link leads to before the boundary answers', async () => {
    ...
    await check(await made(), 'touch through', '', { stderr: "touch: cannot touch 'through': Not a directory\n", exitCode: 1, ...at })
    await check(await made(), 'touch spelled', '', { stderr: "touch: cannot touch 'spelled': Read-only file system\n", exitCode: 1, ...at })
  })
  ...
  it('leaves a link alone where -n has left its destination alone', async () => {
    ...
    await gap(partial, 'cp -rn e /tmp/dest', 'symbolic link', refusal)
    await check(partial, 'find /tmp -type f', '/tmp/dest/e/f\n', at)
  })
  ...
  it('patches through a link on the way, and patches no link itself', async () => {
    ...
    const refused = await gap(await made(), 'patch out < /tmp/d.patch', 'read-only target', 'patch: out.rej: file system is read-only\n')
    ...
    await check(await made(), 'cat /tmp/out', 'a\n', at)
  })
  ...
  it('sed guards the file it reads, which a link naming that file is', async () => {
    ...
    for (const name of ['/tmp/out', 'out']) {
      const t = createTerminal(sources, { mount: '/repo', writable: '/tmp/' })
      await t.run("printf 'a\\n' > /tmp/out")
      await gap(t, `sed 's/a/b/' ${name} >> /tmp/out`, 'streaming self-output', refusal)
      await check(t, 'cat /tmp/out', 'a\n', { cwd: '/repo' })
    }
  })
  ...
  it('watches the file a copy will write, where a link names the one it reports on', async () => {
    ...
    const spelled = await gap(await made(), 'printf seed > /tmp/out; cp -v file /tmp/out > /tmp/out', 'copy output buffering', 'cp: buffered verbose output sharing a copied file is not supported\n')
    const linked = await gap(await made(), 'printf seed > /tmp/out; cp -v file out > /tmp/out', 'copy output buffering', 'cp: buffered verbose output sharing a copied file is not supported\n')
    assert.deepEqual(linked.stdout, spelled.stdout)
    await check(await made(), 'printf seed > /tmp/out; cp -v file out; cat /tmp/out', "'file' -> 'out'\nsrc\n", { cwd: '/repo' })
  })
  ...
  it('realpath resolves every link on the way, and -s keeps the name as written', async () => {
    const t = terminal()
    await check(t, 'realpath pkg/index.js', '/node_modules/pkg/index.js\n')
    await check(t, 'realpath node_modules/.bin/cli', '/node_modules/pkg/bin/cli.js\n')
    ...
    await check(t, 'realpath -e pkg', '/node_modules/pkg\n')
    await check(t, 'realpath --relative-to=. pkg/index.js', 'node_modules/pkg/index.js\n')
  })
  ...
  it('hands a wired command the links a directory holds, and the target each one carries', async () => {
    ...
    await check(t, 'probe node_modules/.bin node_modules/.bin/cli', JSON.stringify({
  ...
  it('carries the links of the sources through a writable overlay, which holds none of its own', async () => {
    const t = createTerminal(SOURCES, { mount: '/repo', writable: '/tmp/' })
    await check(t, 'find / -type l', '/repo/node_modules/.bin/cli\n/repo/node_modules/.bin/stale\n/repo/pkg\n', { cwd: '/repo' })
    await check(t, 'printf x > /tmp/f; find /tmp -type l', '', { cwd: '/repo' })
    await check(t, 'cat pkg/index.js > /tmp/copy; cat /tmp/copy', 'module\n', { cwd: '/repo' })
  })
})
