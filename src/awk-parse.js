...
//   program    { begin: [stmt], end: [stmt], rules: [rule], functions }
...
// while / do / for / forin / break / continue / next / nextfile / exit /
// return / delete / empty; awk-run.js executes them by type.
//
...
export class Parser {
  constructor(tokens) {
    ...
    this.loopDepth = 0
    this.context = 'main'  // 'begin' | 'end' | 'main' | 'function'
  }
...
function parseItem(p, program) {
  ...
  if (p.isKw('BEGIN') || p.isKw('END')) {
    const which = p.next().value
    if (!p.is('{')) p.fail(`${which} requires an action: ${which} { ... }`)
    program[which === 'BEGIN' ? 'begin' : 'end'].push(...parseBlock(p, which === 'BEGIN' ? 'begin' : 'end'))
    return
  }
...
function parseIf(p) {
  ...
  // `else` may follow the terminator that ended the then-branch.
  const mark = p.i
  p.skipTerminators()
...
function simpleJump(p, type) {
  if ((type === 'break' || type === 'continue') && p.loopDepth === 0) p.fail(`\`${type}\` is not allowed outside a loop`)
  if ((type === 'next' || type === 'nextfile') && (p.context === 'begin' || p.context === 'end')) p.fail(`\`${type}\` cannot be used in a ${p.context.toUpperCase()} action`)
  p.next()
  endSimple(p)
  return { type }
}
