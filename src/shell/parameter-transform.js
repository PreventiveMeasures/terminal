...
export async function transformParameter(ref, found, ctx, options) {
  ...
  if (ref.operator === ':') {
    try { return { value: await substring(found.value, ref.word, ctx, options) } }
  ...
  const pattern = await options.expand(raw.slice(0, delimiter), { pattern: true })
  const replacement = await options.expand(delimiter < raw.length ? raw.slice(delimiter + 1) : '', { replacement: true })
  return { value: replaceParameter(found.value, pattern, replacement, ref.operator === '//') }
}

async function substring(value, source, ctx, options) {
  ...
  const arithmetic = async (text) => evaluateArithmetic((await options.expand(text, { arithmetic: true })).value, ctx)
  let start = await arithmetic(source.slice(0, delimiter))
  const size = BigInt(value.length)
  if (start < 0n) start += size
  if (start < 0n || start > size) return ''
  let end = size
  if (delimiter < source.length) {
    const length = await arithmetic(source.slice(delimiter + 1))
